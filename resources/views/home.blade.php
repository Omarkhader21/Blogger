<x-app-layout meta-title="Blogsprint" meta-description="Blogsprint">
                            ...
                            <div class="flex gap-4 mb-2">
                                @foreach ($post->categories as $category)
                                    <a href="{{ route('by-category', $category) }}"
                                        class="bg-blue-500 text-xs text-white font-bold uppercase p-1 rounded">{{ $category->title }}</a>
                                @endforeach
                            </div>
                            <div class="text-xs">
                                {{ $post->shortBody(10) }}
    ...
    @foreach ($categories as $category)
        <div class="mb-3">
            <h2 class="text-lg sm:text-xl font-bold text-blue-500 uppercase pb-1 border-b-2 border-blue-500 mb-3">
                {{ $category->title }} <a href="{{ route('by-category', $category) }}"><i class="fas fa-arrow-right"></i></a>
            </h2>
            <div class="mb-6">
